...
This work uses two data sources:

**1. CRYSTAL-FACE CPI imagery** — High-resolution ice crystal images from 
the 2002 NASA Cirrus Regional Study of Tropical Anvils and Cirrus Layers 
(CRYSTAL-FACE) campaign. These images are used to train the VAE and serve 
as the morphological input for the diffusion model.

**2. WRF-HYSPLIT thermodynamic trajectories** — Seven-hour back-trajectories 
of temperature, pressure, relative humidity, and microphysical variables, 
generated by HYSPLIT driven by WRF simulations of the CRYSTAL-FACE period. 
These serve as the target thermodynamic histories for the diffusion model.
...
The VAE is trained on 128×128 CPI ice crystal images to learn a 
50-dimensional latent representation of crystal morphology. See `vae/` for 
training scripts and architecture details.

**Stage 2: Dataset generation**  
Once the VAE is trained, latent embeddings are extracted for all crystals 
in the dataset. These embeddings are concatenated with 14 geometric and 
image-quality attributes (height, width, aspect ratio, circularity, etc.) 
to form a 64-dimensional conditioning vector.

**Stage 3: Diffusion model training and inference**  
The conditional diffusion model is trained on (conditioning vector, 
trajectory) pairs to generate ensembles of plausible thermodynamic histories 
consistent with observed crystal morphology.
